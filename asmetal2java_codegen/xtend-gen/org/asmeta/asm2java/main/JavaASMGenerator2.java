...
public class JavaASMGenerator2 extends AsmToJavaGenerator {
    ...
    for (final Function fd : _function) {
      if (((fd instanceof MonitoredFunction) || (fd instanceof ControlledFunction))) {
        ...
        if ((_codomain instanceof EnumTd)) {
          ...
          _builder_1.append("()){");
          _append_1.append(_builder_1);
          EList<EObject> codomainContents = fd.getCodomain().eContents();
          for (final EObject enumerativeLog : codomainContents) {
            {
              int _indexOf = enumerativeLog.toString().indexOf("symbol: ");
              int _length = "symbol: ".length();
              final int startIndex = (_indexOf + _length);
              final int endIndex = enumerativeLog.toString().indexOf(")", startIndex);
              final String symbol = enumerativeLog.toString().substring(startIndex, endIndex);
              sb.append(System.lineSeparator());
              StringBuffer _append_2 = sb.append("\t\t\t");
              StringConcatenation _builder_2 = new StringConcatenation();
  ...
  public void controllerGetter(final Asm asm, final StringBuffer sb) {
    String asmName = asm.getName();
    EList<Function> _function = asm.getHeaderSection().getSignature().getFunction();
    for (final Function fd : _function) {
